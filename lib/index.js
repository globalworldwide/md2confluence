...
async function processOnePage(rpConfluence, prefix, pageData) {
  ...
  const update = {
    ...
    version: {
      ...
      message: `Updated by md2confluence from ${pageData.mdfile}`,
    },
  };
...
async function main() {
  ...
  for (const pageData of pages) {
    try {
      // convert page from markdown and upload to Confluence
      await processOnePage(rpConfluence, prefix, pageData);
    } catch (e) {
      // don't let a single bad page derail things
      logger.error(`Error processing page ${pageData.title}`, e);
      // but make sure that the job doesn't appear to complete successfully
      process.exitCode += 1;
    }
  }
}
